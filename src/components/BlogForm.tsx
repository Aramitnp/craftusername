...
export default function BlogForm({ initialData }: BlogFormProps) {
  ...
  useEffect(() => {
    fetch("/api/blog/categories").then(res => res.json()).then(data => Array.isArray(data) && setCategories(data)).catch(console.error);
    fetch("/api/blog/tags").then(res => res.json()).then(data => Array.isArray(data) && setTags(data)).catch(console.error);
  }, []);
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "2rem" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: "2rem" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: "2rem" }}>
          <Card>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "1rem", marginTop: "1rem" }}>
              <div>
                <label className="label-md">Blog Title</label>
                <Input
              ...
              <div>
                <label className="label-md">URL Slug (e.g. blog-post-slug)</label>
                <Input
                  value={formData.slug}
                  onChange={(e) => setFormData({ ...formData, slug: e.target.value })}
                  required
                />
                <span style={{ fontSize: "0.75rem", color: "var(--color-on-surface-variant)" }}>Will be mapped to /blog/slug</span>
              </div>
              <div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "2rem" }}>
          <Card>
            <h3 className="headline-sm">Post Details</h3>
            <div style={{ display: "flex", flexDirection: "column", gap: "1rem", marginTop: "1rem" }}>
              <div>
              ...
              <div>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem", marginTop: "0.5rem" }}>
                  ...
                  {tags.length === 0 && <span style={{ fontSize: "0.875rem", color: "var(--color-on-surface-variant)" }}>No tags available</span>}
                </div>
              </div>
              <div>
                <label className="label-md">Author Name</label>
                <Input
